            ...
            <div class="jobdetail-postjob">
                ...
                <div class="jobdetail-ad-main">
                    <div class="jobdetail-ad-fields">
                        ...
                        <section class="job-similar">
                            <h2>More {{$job->recruiter->company_name}} jobs</h2>
                            <ul>
                    ...
                    @php
                        if($job->recruiter->company_slug == null){
                            $job->recruiter->company_slug = $recruiter->createSlug($recruiter->company_name, $recruiter->cname, $recruiter->id);
                            $job->recruiter->save();
                        }
                        $slug = $job->recruiter->company_slug; @endphp
                    <div class="jobdetail-ad-main-detail">
                        <div class="jobad-btnsave save-btn" data-jobid="{{$job->id}}">
                            <a href="#">save <span><i class="@if(is_array($saved_jobs) && in_array($job->id,$saved_jobs)) fas @else far @endif fa-heart"></i></span></a>
                        </div>
                        <div class="jobad-details-form">
                            ...
                            <div class="jobad-posted-main">
                                <div class="jobs-posted-left">
                                    <div class="jobad-posted-details">
                                        <ul class="posting-details">
                                            <li>Posted by <a class="custom-color" href="{{url('company/'.$slug)}}">{{$job->recruiter->company_name ?? 'N/A'}} </a></li>
                                            <li> {{$job->created_at->diffForHumans()}}</li>
                                            <li><i class="fas fa-map-marker-alt mr-1"></i>{{$job->get_city->name ?? 'N/A'}}</li>
                                            <li><i class="fas fa-money-bill-wave mr-1"></i>{{$settings->symbol}}{{number_format($job->salary_min,2)}} - {{number_format($job->salary_max,2)}} / {{$job->salary_schedule}}</li>
                                        </ul>
                                ...
                                <div class="jobad-logo-main">
                                    <img src="{{asset('recruiters/profile/square_'.$job->recruiter->company_logo)}}" loading="lazy" class="img-fluid" width="70">
                            ...
                            @if($job->job_status == 'expired')
                                <div class="job-ad-apply-head" >
                                    ...
                                    <div class="job-ad-apply-item2-applied" style="background-color: #bb0c0c;">
                                        Expired on {{$job->expiry_date->format("F, d Y")}}
                                    </div>
                        ...
                        <div class="job-detail-btn-links">
                            <div class="jobdetail-btn1 save-btn" data-jobid="{{$job->id}}">
                                <a href="#"><span><i class="@if(is_array($saved_jobs) && in_array($job->id,$saved_jobs)) fas @else far @endif  fa-heart"></i></span>save</a>
                            </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content pad-10">
                <div class="modal-body pt-20">
                    ...
                    <h6>{{$job->recruiter->company_name ?? 'N/A'}} - <span class="font-color">{{$job->get_city->name ?? 'N/a'}}</span></h6>
                    <hr>
